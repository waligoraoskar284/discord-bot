...
module.exports = (client) => {
  ...
  // Usuwa WSZYSTKIE wiadomości (od kogokolwiek) zawierające menu ticketowe lub embed "Wybierz kategorię"
    ...
    try {
      ...
      const toRemove = messages.filter((m) => {
        const hasTicketComponent = m.components?.some((row) =>
          row.components?.some((c) => typeof c.customId === 'string' && c.customId.includes('TICKET_MENU'))
        );
  ...
  const performClose = async (channelId, closedByUserId) => {
    try {
      ...
      // przenieś do archiwum (bez publicznych komunikatów w kanale)
    ...
    try {
      ...
      setInterval(async () => {
        ...
        const hasBotMenu = fetched && fetched.some(m =>
          ...
          m.components?.some((row) => row.components?.some((c) => typeof c.customId === 'string' && c.customId.includes('TICKET_MENU')))
        );
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    try {
      ...
      if (msgSetupUser && msgSetupUser.id !== client.user.id) {
        // Usuń starą wiadomość z ograniczoną sesją, wyślij nową i poinformuj użytkownika ephemeral
        await interaction.message.delete().catch(() => {});
        if (interaction.channel) await sendSetupMessage(interaction.channel).catch(() => {});
        return interaction.reply({
          ephemeral: true,
          content: '❗ Stare menu było ograniczone do innej sesji. Wysłano nowe menu przez bota — kliknij w nie, aby otworzyć ticket.'
        }).catch(() => {});
      }
    } catch (err) {
