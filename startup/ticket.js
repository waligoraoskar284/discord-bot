/**
 * Zaktualizowany skrypt ticketów (polski)
 *
 * Zmiany względem poprzedniej wersji:
 * - "Panel administracyjny" otwiera teraz pierwsze menu (ephemeral) z kategoriami akcji.
 * - Po kliknięciu kategorii (np. "Moderacja") pojawia się kolejne menu (ephemeral) z konkretnymi opcjami
 *   (np. Zbanuj, Wyrzuć, Ostrzeż). To realizuje "menu -> podmenu" zgodnie z prośbą.
 * - Istniejące akcje (ban, kick, warn, giverole, force close) nadal działają — potwierdzenia/modal są
 *   wyświetlane tak jak wcześniej.
 * - Wszystkie komunikaty są po polsku.
 *
 * Uwaga:
 * - Upewnij się, że ID (TICKET_CHANNEL_ID, ADMIN_ROLE_ID, ...) są poprawne.
...
module.exports = (client) => {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    const parts = interaction.customId.split('::'); // [modal_type, userId, ticketNumber] lub modal_warn/modal_giverole
    if (parts.length < 2) {
      await interaction.reply({ content: '❌ Nieprawidłowy formularz.', ephemeral: true });
      return;
    }

    // Obsługa warn/giverole modalów później
    const modalType = parts[0];

    if (modalType === 'modal_warn' || modalType === 'modal_giverole') {
      // te modal'e obsługiwane w dedykowanym bloku poniżej
      return;
    }
  ...
  // --- Button interactions (główny handler) ---
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // Pobierz dane ticketa (fallback)
    const data = await getTicketData(channelId, interaction.guild);
    if (!data && !action.startsWith('admin_panel') && !action.startsWith('admin_main')) {
    ...
    const isOwner = ownerId ? (member.id === ownerId) : false;
    ...
    if (action === 'close_ticket' || action === 'close') {
      ...
      if (data) {
        ...
        ticketData.set(channelId, { ownerId: null, transactionId: null, ticketNumber: null, category: null, createdAt: new Date().toISOString(), closeTimeoutId: null, closeConfirmTimeoutId: timeoutId });
      }
      return;
    }
    ...
    // --- Panel administracyjny: pierwsze menu (kategorie) ---
    if (action === 'admin_panel' || action === 'admin') {
      ...
      const adminMainEmbed = new EmbedBuilder()
        ...
        .setDescription('Wybierz kategorię działań dotyczących tego ticketu.')
        .setColor('Purple')
        .addFields(
          { name: 'Użytkownik', value: `<@${ownerId || 'brak'}>`, inline: true },
          { name: 'Kanał', value: `<#${channelId}>`, inline: true }
        );

      // KATEGORIE: Moderacja, Role & Informacje, Zarządzanie ticketem
    ...
    // --- Podmenu: Moderacja (ban/kick/warn) ---
    ...
    // --- Podmenu: Zarządzanie ticketem (zamknij, usuń) ---
    ...
    // --- Pozostałe admin akcje - PRESERVE existing flows ---
    if (action === 'admin_ban') {
