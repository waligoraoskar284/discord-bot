...
module.exports = (client) => {
  ...
  const performClose = async (channelId, closedByUserId) => {
    try {
      ...
      await ch.send({ content: `🔒 Ticket został zamknięty przez <@${closedByUserId}>.` }).catch(() => {});
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    const user = interaction.user;
    // Usuń walidację użytkownika (żeby inni nie blokowali) - ale zostaw jeśli chcesz stricte walidować:
    if (user.id !== userIdFromModal) {
      // Nie blokujemy - pozwalamy (poprzednio był błąd). Tylko powiadomimy jeśli inny user submited (bez przerwania).
      // Możemy jednak nadal przerwać, ale user chciał, żeby działało szerzej.
      // Kontynuujemy bez zwracania błędu.
    }
    ...
    // Wyślij log
    await sendLog('Utworzono ticket', `Utworzono ticket ${ticketNumberCounter}`, [
      { name: 'Ticket', value: `#${ticketNumberCounter} | ${category}` },
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (!data) {
      await interaction.reply({ content: '❌ Nie znaleziono danych dla tego ticketa (prawdopodobnie został już zamknięty). Spróbuj ponownie /setup jeśli problem występuje.', ephemeral: true }).catch(() => {});
      return;
    }
    ...
    if (action === 'close_ticket' || action === 'close') {
      ...
      await interaction.reply({ content: '🔔 Ticket zostanie zamknięty za 5 sekund. Kliknij "Anuluj" aby przerwać.', components: [confirmRow], ephemeral: true }).catch(() => {});
    ...
    if (action === 'confirm_close') {
      ...
      await performClose(channelId, interaction.user.id);
      await interaction.reply({ content: '✅ Ticket został zamknięty natychmiast.', ephemeral: true });
      return;
    }
