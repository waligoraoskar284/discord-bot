 ...
 *
 * Poprawiony moduł ticketów:
 * - jedno spójne ephem. potwierdzenie (deferReply + editReply)
 * - jeśli w DB jest ticket, ale kanał już nie istnieje -> wpis jest czyszczony
 * - maks. 1 otwarty ticket na użytkownika w danej kategorii
 *
 * Użycie:
...
module.exports = function initTicket(client, userConfig = {}) {
  ...
  // create ticket: ensures only one open ticket per (user, category) and cleans stale DB entries
  async function createTicketChannel(interaction, category, formData) {
    const guild = interaction.guild;
    const author = interaction.user;

    // Check existing open tickets for this user & category in this guild
    for (const [chId, t] of Object.entries(db.tickets || {})) {
      if (t.guildId === guild.id && t.userId === author.id && t.category === category && !t.archivedAt) {
        ...
        if (existingCh) {
          ...
          // channel missing -> cleanup DB entry and allow new ticket
          delete db.tickets[chId];
          saveDB(db);
          // continue loop in case there are other leftover entries (unlikely)
        }
      }
    }
  ...
  if (!client._ticketModuleInitialized) {
    ...
    client.on('interactionCreate', async (interaction) => {
      try {
        ...
        // modal submit -> deferReply once, then create ticket and edit reply (single embed)
          ...
          if (interaction.customId === 'modal_INNE') {
            ...
            if (res.alreadyOpen) {
              ...
              const alreadyEmbed = new EmbedBuilder()
                ...
                .setColor(0xffcc00)
                .setFooter({ text: '© 2025r. Sprawdziany & Kartkówki', iconURL: CONFIG.FOOTER_LOGO_URL });
