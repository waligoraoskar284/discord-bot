...
module.exports = (client) => {
  ...
  // usuwa WSZYSTKIE wiadomości z podanego kanału (ostrożnie — rate limits, starsze niż 14 dni trzeba usuwać pojedynczo)
  const deleteAllMessagesInChannel = async (channel) => {
    try {
      ...
      do {
        ...
        // usuwamy pojedynczo aby uniknąć problemów z bulkDelete dla starych wiadomości
        for (const msg of fetched.values()) {
          try { await msg.delete().catch(() => {}); } catch (e) { /* ignore */ }
        }
        // jeżeli było <100 to pętla się zakończy, w przeciwnym razie spróbujemy pobrać kolejne
      } while (fetched && fetched.size === 100);
    } catch (err) {
      console.error('[ticket] deleteAllMessagesInChannel error:', err);
    }
  };
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (action === 'deploy_latest_commit') {
      ...
      try {
        await deleteAllMessagesInChannel(targetCh);
