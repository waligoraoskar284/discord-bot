...
module.exports = (client) => {
  ...
  const isSetupMessage = (m) => {
    try {
      if (!m) return false;
      if (!m.author || m.author.id !== client.user?.id) return false; // tylko nasze wiadomości
  ...
  // usuń tylko wiadomości SETUP wysłane przez bota (używane przy starcie / deploy)
  ...
  const sendSetupMessage = async (channel) => {
    ...
    if (sent) {
      ...
      // usuń tylko inne stare wiadomości bota typu setup (pozostaw tę nową)
      // pobierz ostatnie 200 i usuń wszystkie setupowe poza sent.id
  ...
  client.once(Events.ClientReady, async () => {
    try {
      ...
      // periodic cleaner co 5 minut: upewnia się że jest max 1 wiadomość setup (botowa)
      setInterval(async () => {
        ...
        if (setupMessages.size > 1) {
          ...
          for (let i = 0; i < sorted.length - 1; i++) { try { await sorted[i].delete().catch(() => {}); } catch (e) {} }
  ...
  // --- Select menu (poprawiona, prostsza obsługa) ---
  client.on(Events.InteractionCreate, async (interaction) => {
    if (!interaction.isStringSelectMenu()) return;
    if (!interaction.customId?.startsWith('TICKET_MENU::ticket_category::')) return;

    // Jeśli komponent NIE pochodzi od naszego bota -> odmowa i informacja (nie usuwamy automatycznie)
    if (interaction.message?.author?.id !== client.user?.id) {
      try {
        await interaction.reply({ content: '❗ Ten przycisk nie pochodzi od aktualnego menu. Skontaktuj się z administracją.', flags: 64 });
      } catch (e) { /* ignore */ }
      return;
    }

    // Jeżeli pochodzi od NASZEGO BOTA -> akceptujemy i synchronizujemy lastSetup (nie usuwamy wiadomości)
    try {
    ...
    // teraz normalna obsługa wyboru kategorii
  ...
  // --- Modal submit -> create ticket channel (bez zmian logicznych) ---
  client.on(Events.InteractionCreate, async (interaction) => {
