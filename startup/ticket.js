...
module.exports = (client) => {
  ...
  // helper: generuj unikalny transaction id (licznik + timestamp)
  const generateTransactionId = () => {
    transactionCounter += 1;
    return `T${transactionCounter}-${Date.now()}`;
  };
  ...
  const performClose = async (channelId, closedByUserId) => {
    try {
      ...
      // Nie wysyłamy publicznych komunikatów do kanału (usuń publiczne wiadomości o przeniesieniu i usunięciu)
      await ch.setParent(CLOSED_CATEGORY_ID).catch(() => {});

      // zaplanuj automatyczne usunięcie po 10h (bez powiadomień w kanale)
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // Jeśli menu pochodzi z czyjejś sesji/komendy (message.interaction.user != bot),
    // usuń tę wiadomość i wyślij własne menu, żeby uniknąć angielskiego "Only the person..." systemowego komunikatu.
    try {
      const msgSetupUser = interaction.message?.interaction?.user;
      if (msgSetupUser && msgSetupUser.id !== client.user.id) {
        // spróbuj usunąć starą wiadomość z niepoprawną sesją
        await interaction.message.delete().catch(() => {});
        // wyślij nową (botową) wiadomość z menu
        await sendSetupMessage(interaction.channel).catch(() => {});
        // poinformuj użytkownika i zakończ - poprosimy kliknąć w nowe menu
        return interaction.reply({
          ephemeral: true,
          content: '❗ Stary setup był ograniczony do innej sesji i został zastąpiony. Kliknij proszę w nowe menu wysłane przez bota, aby utworzyć ticket.'
        }).catch(() => {});
      }
    } catch (err) {
      // ignoruj — dalej normalnie próbujemy obsłużyć interakcję
      console.error('[ticket] Handling foreign setup menu error:', err);
    }
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // Emoji dla nagłówków (zgodnie z prośbą)
    ...
    if (modalType === 'modal_inne') {
      ...
      embedFields = [
        { name: `${emojiUserHeader} **__Informacje o użytkowniku:__**`, value: `${getEmoji('1452951976785481741') || ''} Ping: <@${user.id}>\n${getEmoji('1452712183589900298') || ''} TAG: ${user.tag}\n${getEmoji('1452715580456042647') || ''} ID użytkownika: ${user.id}` },
        {
          ...
          value:
            `${eTransLine} ID transakcji: ${transactionId}\n` +
    ...
    // Zapisz dane ticketa (z transactionId jeśli jest)
    ...
    // Embed z datą i godziną w stopce, author i pola
    ...
    // Wyślij log (rozszerzony) - uwzględniamy transactionId
    await sendLog('Utworzono ticket', `Utworzono ticket #${modalTicketNumber} (${category})`, [
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (action === 'close_ticket' || action === 'close') {
      ...
      if (didDefer) {
        ...
        // gdy nie zdeferrowaliśmy (rzadko), odpisz normalnie ephemeral
    ...
    if (action === 'cancel_close') {
      ...
      ticketData.set(channelId, data);
      try {
        // publicznego ch.send usuwamy — zamiast tego logujemy i wysyłamy ephemeral do klikającego
    ...
    if (action === 'admin_moderation') {
      if (!canUseAdminButtons) return (didDefer ? interaction.editReply({ content: '❌ Nie masz uprawnień.' }) : interaction.reply({ content: '❌ Nie masz uprawnień.', ephemeral: true })).catch(() => {});
      ...
      return (didDefer ? interaction.editReply({ embeds: [moderationEmbed], components: [moderationButtons] }) : interaction.reply({ embeds: [moderationEmbed], components: [moderationButtons], ephemeral: true })).catch(() => {});
    }
    ...
    if (action === 'admin_roles') {
      ...
      return (didDefer ? interaction.editReply({ embeds: [rolesEmbed], components: [rolesButtons] }) : interaction.reply({ embeds: [rolesEmbed], components: [rolesButtons], ephemeral: true })).catch(() => {});
    }
    ...
    if (action === 'admin_ticketmanage') {
      ...
      return (didDefer ? interaction.editReply({ embeds: [tmEmbed], components: [tmButtons] }) : interaction.reply({ embeds: [tmEmbed], components: [tmButtons], ephemeral: true })).catch(() => {});
    }

    // Pozostałe admin akcje (ban/kick/warn/giverole/force close/viewinfo)
    if (action === 'admin_ban') {
      if (!canUseAdminButtons) return (didDefer ? interaction.editReply({ content: '❌ Nie masz uprawnień.' }) : interaction.reply({ content: '❌ Nie masz uprawnień.', ephemeral: true })).catch(() => {});
      const confirmRow = new ActionRowBuilder().addComponents(
      ...
      return (didDefer ? interaction.editReply({ content: `Potwierdź zbanowanie użytkownika <@${ownerId}>.`, components: [confirmRow] }) : interaction.reply({ content: `Potwierdź zbanowanie użytkownika <@${ownerId}>.`, components: [confirmRow], ephemeral: true })).catch(() => {});
    }
    ...
    if (action === 'confirm_ban_yes') {
      ...
      try {
        ...
        return (didDefer ? interaction.editReply({ content: `✅ Użytkownik <@${ownerId}> został zbanowany.` }) : interaction.reply({ content: `✅ Użytkownik <@${ownerId}> został zbanowany.`, ephemeral: true })).catch(() => {});
      } catch (err) {
        console.error('[ticket] ban error:', err);
        return (didDefer ? interaction.editReply({ content: '❌ Nie udało się zbanować użytkownika. Sprawdź uprawnienia bota.' }) : interaction.reply({ content: '❌ Nie udało się zbanować użytkownika. Sprawdź uprawnienia bota.', ephemeral: true })).catch(() => {});
      }
    }
    ...
    if (action === 'admin_kick') {
      ...
      return (didDefer ? interaction.editReply({ content: `Potwierdź wyrzucenie użytkownika <@${ownerId}>.`, components: [confirmRow] }) : interaction.reply({ content: `Potwierdź wyrzucenie użytkownika <@${ownerId}>.`, components: [confirmRow], ephemeral: true })).catch(() => {});
    }
    ...
    if (action === 'confirm_kick_yes') {
      ...
      try {
        ...
        if (!memberToKick) return (didDefer ? interaction.editReply({ content: '❌ Nie znaleziono członka.' }) : interaction.reply({ content: '❌ Nie znaleziono członka.', ephemeral: true })).catch(() => {});
        ...
        return (didDefer ? interaction.editReply({ content: `✅ Użytkownik <@${ownerId}> został wyrzucony.` }) : interaction.reply({ content: `✅ Użytkownik <@${ownerId}> został wyrzucony.`, ephemeral: true })).catch(() => {});
      } catch (err) {
        console.error('[ticket] kick error:', err);
        return (didDefer ? interaction.editReply({ content: '❌ Nie udało się wyrzucić użytkownika. Sprawdź uprawnienia bota.' }) : interaction.reply({ content: '❌ Nie udało się wyrzucić użytkownika. Sprawdź uprawnienia bota.', ephemeral: true })).catch(() => {});
      }
    }

    if (action === 'confirm_no') {
      return (didDefer ? interaction.editReply({ content: '❌ Anulowano operację.' }) : interaction.reply({ content: '❌ Anulowano operację.', ephemeral: true })).catch(() => {});
    }

    if (action === 'admin_warn') {
      if (!canUseAdminButtons) return interaction.reply({ content: '❌ Nie masz uprawnień.', ephemeral: true }).catch(() => {});
      // nie deferujemy przed showModal!
      ...
      try {
        ...
        return interaction.reply({ content: '❌ Nie udało się otworzyć formularza ostrzeżenia.', ephemeral: true }).catch(() => {});
      }
    }
    ...
    if (action === 'admin_giverole') {
      ...
      try {
        ...
        return interaction.reply({ content: '❌ Nie udało się otworzyć formularza dodania roli.', ephemeral: true }).catch(() => {});
      }
    }
    ...
    if (action === 'admin_force_close') {
      ...
      try {
        ...
        return (didDefer ? interaction.editReply({ content: '✅ Ticket usunięty siłowo.' }) : interaction.reply({ content: '✅ Ticket usunięty siłowo.', ephemeral: true })).catch(() => {});
      } catch (err) {
        console.error('[ticket] force close error:', err);
        return (didDefer ? interaction.editReply({ content: '❌ Nie udało się usunąć ticketa.' }) : interaction.reply({ content: '❌ Nie udało się usunąć ticketa.', ephemeral: true })).catch(() => {});
      }
    }
    ...
    if (action === 'admin_viewinfo') {
      ...
      try {
        ...
        return (didDefer ? interaction.editReply({ embeds: [new EmbedBuilder().setTitle('Informacje o użytkowniku').addFields(info).setColor('Blue')] }) : interaction.reply({ embeds: [new EmbedBuilder().setTitle('Informacje o użytkowniku').addFields(info).setColor('Blue')], ephemeral: true })).catch(() => {});
      } catch (err) {
        console.error('[ticket] viewinfo error:', err);
        return (didDefer ? interaction.editReply({ content: '❌ Błąd podczas pobierania informacji.' }) : interaction.reply({ content: '❌ Błąd podczas pobierania informacji.', ephemeral: true })).catch(() => {});
      }
    }
