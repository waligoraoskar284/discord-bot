...
module.exports = (client) => {
  ...
  const cleanSetupMessages = async (channel) => {
    try {
      ...
      // zbieramy wiadomości pasujące do wzorca z kilku fetch'ów (do 100 na raz) i usuwamy je
      do {
        ...
        // jeśli mamy <100 to pętla zakończy się
      } while (lastId);
    } catch (err) {
  ...
  client.once(Events.ClientReady, async () => {
    try {
      ...
      if (newMsg) {
        lastSetup = { messageId: newMsg.id, token: lastSetup.token || null };
      }
      ...
      setInterval(async () => {
        ...
        // pobierz ostatnie 200 wiadomości i wyszukaj wszystkie wiadomości typu setup
        ...
        if (setupMessages.size === 0) {
          ...
          if (sent) lastSetup = { messageId: sent.id, token: lastSetup.token || null };
          return;
        }

        // Jeżeli jest więcej niż 1 -> zostaw najnowszą, usuń pozostałe
        if (setupMessages.size > 1) {
          // sortuj wg createdTimestamp rosnąco, weź ostatni jako newest
          ...
          // ustaw lastSetup na newest (pobrany)
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (action === 'deploy_latest_commit') {
      // tutaj wymagamy uprawnień admina/modów lub ManageGuild
