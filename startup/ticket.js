...
module.exports = (client) => {
  ...
  const sendSetupMessage = async (channel) => {
    ...
    // UWAGA: wysyłamy zwykłą wiadomość przez channel.send (nie interaction), aby nikt nie miał ograniczeń "Only the person who initiated..."
  ...
  // --- Modal submit -> create ticket channel / admin modals handled below ---
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // Admin modals handled later
    if (modalType === 'modal_warn' || modalType === 'modal_giverole') return;
    ...
    // Rozszerzony log przy tworzeniu ticketu (z przyciskiem "Otwórz ponownie ticket" tylko po zamknięciu - tutaj dodajemy log bez button)
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (action === 'log_reopen') {
      ...
      // Odróznij czy kanał jest w kategorii zamkniętych - przenieś do kategorii ticketów i przywróć uprawnienia właściciela
      try {
        await ch.setParent(TICKETS_CATEGORY_ID).catch(() => {});
        // Odbuduj dane jeśli nie ma
        ...
        // przywróć pisanie dla właściciela
        if (stored.ownerId) {
    ...
    if (action === 'admin_panel' || action === 'admin') {
      ...
      // Rozszerzone informacje o użytkowniku/tickecie
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (id.startsWith('modal_warn::')) {
      ...
      try {
        // 1) Wyślij DM do użytkownika z embedem (czerwony)
        ...
        if (warnedMember) {
          await warnedMember.send({ embeds: [warnEmbed] }).catch(() => {
            // DM mógł być zablokowany - wyślij tylko log do kanału logów
            console.warn('[ticket] Nie udało się wysłać DM z ostrzeżeniem (użytkownik ma zablokowane DMy).');
          });
        }

        // 2) Nie wysyłamy publicznej wiadomości do kanału ticketu "Użytkownik X został ostrzeżony."
        // Zamiast tego: potwierdzenie dla moderatora
        await interaction.reply({ content: `✅ Ostrzeżenie wysłane prywatnie do <@${data.ownerId}>.`, ephemeral: true });

        // 3) Wyślij rozszerzony log do kanału LOG_CHANNEL_ID z embedem i przyciskiem do ponownego otwarcia
        ...
        await sendLog('Ostrzeżenie', '', [], 'Red', []); // opcjonalnie: pusta wysyłka, ale lepiej wysłać pełny embed poniżej
        // Wyślij bezpośrednio do log channel, aby mieć pełną kontrolę nad embedem i przyciskiem:
