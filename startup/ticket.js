...
module.exports = (client) => {
  ...
  const performClose = async (channelId, closedByUserId) => {
    try {
      ...
      try {
        // Najpierw odbierz widoczność właścicielowi - aby na pewno nie widział w archiwum
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // JEŻELI SELECT NALEŻY DO INNEJ SESJI SETUP (ktoś inny uruchomił /setup) - obsłużmy to po polsku, żeby nie pokazywał się systemowy komunikat
    try {
      const msgSetupUser = interaction.message?.interaction?.user;
      if (msgSetupUser && msgSetupUser.id !== client.user.id) {
        // Wiadomość z menu została utworzona przez inną sesję/komendę - obsłuż spokojnie
        await interaction.reply({
          ephemeral: true,
          content: '❌ Ten wybór pochodzi z czyjejś sesji setup i jest ograniczony. Uruchom /setup samodzielnie albo poproś osobę, która uruchomiła setup, aby go zresetowała.'
        }).catch(() => {});
        return;
      }
    } catch (err) {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // Przydatne emoji (zgodnie z prośbą)
    const eHeader = getEmoji('1453068281979076691') || '';
    const ePing = getEmoji('1452951976785481741') || '';
    const eTag = getEmoji('1452712183589900298') || '';
    const eUserId = getEmoji('1452715580456042647') || '';
    const eTrans = getEmoji('1453070829285019658') || ''; // emoji dla pól transakcji
    ...
    if (modalType === 'modal_inne') {
      ...
      // Pole "Informacje o użytkowniku" - wyróżnione (jako bold w nazwie pola) + emoji
      embedFields = [
        { name: `${eHeader} **Informacje o użytkowniku**`, value: `${ePing} Ping: <@${user.id}>\n${eTag} TAG: ${user.tag}\n${eUserId} ID użytkownika: ${user.id}` },
        { name: `${eHeader} **Opis problemu**`, value: `> ${opis}` }
      ];
    } else {
      ...
      embedFields = [
        { name: `${eHeader} **Informacje o użytkowniku**`, value: `${ePing} Ping: <@${user.id}>\n${eTag} TAG: ${user.tag}\n${eUserId} ID użytkownika: ${user.id}` },
        {
          ...
          value:
            `${eTrans} ID transakcji: ${transactionId}\n` +
            `${eTrans} Kategoria: ${category}\n` +
    ...
    // Embed z datą i godziną w stopce, autor (avatar) i ładne pola
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (action === 'cancel_close') {
      if (!data.closeConfirmTimeoutId) {
        return interaction.editReply({ content: '❌ Brak zaplanowanego zamknięcia.' }).catch(() => {});
      }
    ...
    if (action === 'confirm_close') {
      if (!isOwner && !canUseAdminButtons) {
        return interaction.editReply({ content: '❌ Nie masz uprawnień.' }).catch(() => {});
      }
      if (data.closeConfirmTimeoutId) {
        clearTimeout(data.closeConfirmTimeoutId);
        data.closeConfirmTimeoutId = null;
    ...
    if (action === 'admin_panel' || action === 'admin') {
      if (!canUseAdminButtons) {
        return interaction.editReply({ content: '❌ Nie masz uprawnień do otwarcia panelu administracyjnego.' }).catch(() => {});
      }
