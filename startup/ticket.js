...
module.exports = (client) => {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // Jeśli wiadomość pochodzi od NASZEGO BOTA -> akceptujemy interakcję bez porównywania lastSetup.token
    try {
      if (interaction.message && interaction.message.author && interaction.message.author.id === client.user?.id) {
        // synchronizujemy localny stan z tym message id i tokenem (przydatne po restarcie/odświeżeniu)
    ...
    // dalej normalna obsługa wyboru kategorii
    try {
      // nie blokujemy interakcji przez usuwanie menu — po prostu pokazujemy modal
      // (jeśli chcesz dodatkową walidację na token, można dodać, ale powodowało to niechciane usuwanie)
    } catch (e) {
      console.error('[ticket] select pre-check error:', e);
    }
    ...
    // showModal musi być wywołane bez wcześniejszego defer
    await interaction.showModal(modal).catch(async (err) => {
      console.error('[ticket] showModal error:', err);
      try { await interaction.reply({ content: '❌ Nie udało się otworzyć formularza. Spróbuj ponownie.', ephemeral: true }); } catch {}
    });
  });
  ...
  // --- Button interactions (niezmienione, poza drobnymi sync) ---
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // ... pozostała obsługa buttonów (close, admin_panel, deploy itp.) — bez zmian względem wersji powyżej
    // (dla skrótu w tym pliku powyżej jest pełna implementacja — w Twoim pliku ta sekcja pozostaje taka sama)
    // Dla kompletności zachowujemy wcześniejsze behavior — jeżeli potrzebujesz, mogę wstawić tutaj całość ponownie.
