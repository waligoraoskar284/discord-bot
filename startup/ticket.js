...
module.exports = (client) => {
  ...
  const LOG_CHANNEL_ID = '1454069542283120642'; // <-- docelowy kanał logów
  ...
  const sendLog = async (title, description = '', fields = [], color = 'Blue', components = [], embedOverride = null) => {
    try {
      ...
      // Dla bezpieczeństwa sprawdzamy, czy ma metodę send (TextChannel, NewsChannel, itp.)
      ...
      if (!embedOverride && fields.length) {
        // fields powinny być tablicą { name, value, inline? }
        try {
      ...
      await logCh.send({ embeds: [embed], components }).catch((err) => {
        // Gdy wysyłanie się nie uda - logujemy to do konsoli
