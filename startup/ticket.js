...
module.exports = (client) => {
  ...
  // nazwa pozostawiona dla kompatybilności; używana tylko przy starcie
  ...
  // --- Select menu: TYLKO tworzymy i pokazujemy modal; NIE USUWAM ani NIE ODŚWIEŻAM wiadomosci setup ---
  client.on(Events.InteractionCreate, async (interaction) => {
    if (!interaction.isStringSelectMenu()) return;
    if (interaction.customId !== 'TICKET_MENU::ticket_category') return;
    if (interaction.message.author.id !== client.user.id) return;

    // tworzymy modal i pokazujemy go — to jedyna operacja w tym handlerze
    try {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // --- ZAMIANA: zamiast prostego tekstu wysyłamy ładnego niebieskiego embeda sukcesu (ephemeral) ---
    try {
