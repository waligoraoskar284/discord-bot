...
module.exports = (client) => {
    ...
    client.once(Events.ClientReady, async () => {
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor('Blue')
                // obrazek informacyjny (tu zostaje logo serwera)
    ...
    client.on(Events.InteractionCreate, async interaction => {
        ...
        if (interaction.customId.startsWith('modal_inne')) {
            ...
            // Zgodnie z wzorem: najpierw informacje o użytkowniku, potem WYRAŹNY nagłówek "Transaction Information:"
            embedFields = [
                {
                    name: 'User Information:',
                    value:
                ...
                // Pole "Transaction Information" ustawione jako nazwa pola (nazwy pól są wyraźniejsze niż treść),
                // dodatkowo poprzedzamy je emoji, aby były bardziej widoczne.
                {
        ...
        // Ważna zmiana: miniature ticketa (po prawej) - avatar użytkownika, nie logo serwera
        const ticketEmbed = new EmbedBuilder()
            .setTitle(`Ticket #${ticketCounter} | ${category}`)
            .setColor(category === 'ZAKUPY' ? 'Red' : 'Orange')
            .setFields(embedFields)
            .setThumbnail(user.displayAvatarURL({ dynamic: true, size: 1024 })); // <- tutaj avatar użytkownika
