...
module.exports = (client) => {
  ...
  // Usuwa WIADOMOŚCI (od kogokolwiek) zawierające menu ticketowe lub embed "Wybierz kategorię".
  ...
  // Sprawdza, czy użytkownik ma już otwarty ticket w danej kategorii (skanuje kanały w kategorii ticketów)
  ...
  // Zamknięcie ticketu (przeniesienie do archiwum + log + automatyczne usunięcie)
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    try {
      ...
      // Usuń tamtą wiadomość (jeśli istnieje), wyślij nowe menu przez bota i poinformuj użytkownika po polsku.
      if (!token || token !== lastSetup.token) {
        // Najpierw ackujemy interakcję (odp. ephemeral) żeby uniknąć systemowego angielskiego komunikatu od Discorda,
        // następnie usuwamy starą wiadomość i wysyłamy nowe menu.
        try {
          if (!interaction.replied && !interaction.deferred) {
            await interaction.reply({
              ephemeral: true,
              content: '❗ Ta wiadomość setup jest nieaktualna (pochodzi z innej sesji). Wysłałem nowe menu przez bota — kliknij w nie, aby utworzyć ticket.'
            });
          }
        } catch (e) {
          // jeśli nie uda się normalnie odpisać, spróbuj tylko zdeferować (bez treści)
          try { await interaction.deferReply({ ephemeral: true }).catch(() => {}); } catch (e2) { /* ignore */ }
        }

        // Usuń starą ograniczoną wiadomość (aby nie generowała systemowego komunikatu przy kliknięciu)
        try { await interaction.message.delete().catch(() => {}); } catch (e) { }

        // Wyślij nowe menu botowe do tego kanału (zaktualizuje lastSetup)
        if (interaction.channel) await sendSetupMessage(interaction.channel).catch(() => {});
        return;
      }
    } catch (err) {
      console.error('[ticket] Handling foreign/old setup menu error:', err);
      // jeśli coś poszło nie tak, pozwól przejść dalej (ale ryzyko systemowego komunikatu jest minimalne, bo usuwamy stare)
    }
