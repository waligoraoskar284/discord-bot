...
module.exports = (client) => {
  ...
  // Jeżeli masz ID roli admina, uzupełnij tutaj zamiast null (np. process.env.ADMIN_ROLE_ID)
  ...
  // rozpoznaje czy wiadomość jest naszym "setup" (menu/select lub embed z tytułem "Wybierz kategorię")
  const isSetupMessage = (m) => {
    try {
      if (!m) return false;
      // tylko wiadomości wysłane przez bota traktujemy jako setup
  ...
  // pomocnik: z parsuje token z komponentu w istniejącej wiadomości (jeśli jest)
  const parseTokenFromMessage = (msg) => {
    try {
      if (!msg || !msg.components) return null;
      for (const row of msg.components) {
        for (const comp of row.components) {
          if (comp?.customId && comp.customId.startsWith('TICKET_MENU::ticket_category::')) {
  ...
  // czyści WSZYSTKIE wiadomości typu setup w kanale (bez zachowywania żadnej) - używane przy starcie aby wymusić świeżą wiadomość
  ...
  // usuwa wszystkie stare wiadomości setup poza keepMessageId (jeżeli keepMessageId podane - pozostawia tę jedną)
  ...
  client.once(Events.ClientReady, async () => {
    try {
      ...
      // --- NA STARCIE: USUŃ WSZYSTKIE ISTNIEJĄCE WIADOMOŚCI SETUP I WYŚLIJ NOWĄ ---
      await cleanSetupMessages(ch);
      const newMsg = await sendSetupMessage(ch);
      if (newMsg) {
        // sendSetupMessage już ustawiło lastSetup, ale upewniamy się że mamy id i token
        const token = parseTokenFromMessage(newMsg) || lastSetup.token;
        lastSetup = { messageId: newMsg.id, token };
      }
      ...
      setInterval(async () => {
        ...
        // pobierz ostatnie 200 wiadomości i wyszukaj wszystkie wiadomości typu setup _wysłane przez bota_
        const fetched = await chLoop.messages.fetch({ limit: 200 }).catch(() => null);
        if (!fetched) return;

        const setupMessages = fetched.filter(m => isSetupMessage(m));
        // Jeżeli brak -> wyślij nową
        if (setupMessages.size === 0) {
          const sent = await sendSetupMessage(chLoop).catch(() => null);
          if (sent) {
            const token = parseTokenFromMessage(sent) || null;
            lastSetup = { messageId: sent.id, token };
          }
          return;
        }

        // Jeżeli jest więcej niż 1 -> zostaw najnowszą, usuń pozostałe
        if (setupMessages.size > 1) {
          const sorted = Array.from(setupMessages.values()).sort((a, b) => a.createdTimestamp - b.createdTimestamp);
          const newest = sorted[sorted.length - 1];
          for (let i = 0; i < sorted.length - 1; i++) {
            try { await sorted[i].delete().catch(() => {}); } catch (e) { /* ignore */ }
          }
          lastSetup.messageId = newest.id;
          const token = parseTokenFromMessage(newest);
          if (token) lastSetup.token = token;
          return;
        }

        // setupMessages.size === 1 -> upewnij się, że lastSetup odpowiada tej wiadomości
        if (setupMessages.size === 1) {
          const only = setupMessages.first();
          lastSetup.messageId = only.id;
          const token = parseTokenFromMessage(only);
          if (token) lastSetup.token = token;
        }
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // Jeśli interakcja pochodzi z wiadomości, która NIE jest wysłana przez tego bota,
    // usuń tę wiadomość (jeśli można) i wyślij nową publiczną wiadomość setup,
    // następnie powiadom użytkownika, żeby spróbował jeszcze raz.
    try {
      if (interaction.message && interaction.message.author && interaction.message.author.id !== client.user?.id) {
        try {
        ...
        if (ch) {
          const sent = await sendSetupMessage(ch).catch(() => null);
          if (sent) lastSetup = { messageId: sent.id, token: parseTokenFromMessage(sent) || null };
        }
        // Nie próbujemy dalej obsługiwać tej interakcji — informujemy użytkownika
        try { await interaction.reply({ ephemeral: true, content: '❗ Ten przycisk pochodził z nieprawidłowego menu — wysłałem nowe menu. Spróbuj ponownie.' }); } catch {}
        return;
      }
    } catch (e) { /* ignore */ }

    // Jeśli interakcja pochodzi z wiadomości bota, zaktualizuj lastSetup, żeby token + id były spójne
    try {
      if (interaction.message && interaction.message.author && interaction.message.author.id === client.user?.id) {
        const msgToken = parseTokenFromMessage(interaction.message);
        if (msgToken) {
          lastSetup = { messageId: interaction.message.id, token: msgToken };
        }
      }
    } catch (e) { /* ignore */ }

    // Jeżeli token się nie zgadza -> spróbuj odświeżyć (usuń starą botową) i wyślij nową, poinformuj użytkownika.
    try {
      if (!token || token !== lastSetup.token) {
        // jeśli wiadomość była nasza (stara) - usuń i wyślij nową
        if (interaction.message && interaction.message.author && interaction.message.author.id === client.user?.id) {
          try { await interaction.message.delete().catch(() => {}); } catch (e) { /* ignore */ }
          const ch = interaction.channel || await client.channels.fetch(interaction.channelId).catch(() => null);
          if (ch) {
            const sent = await sendSetupMessage(ch).catch(() => null);
            if (sent) lastSetup = { messageId: sent.id, token: parseTokenFromMessage(sent) || null };
          }
          try { await interaction.reply({ ephemeral: true, content: '❗ Menu było nieaktualne — odświeżyłem je. Spróbuj ponownie.' }); } catch {}
          return;
        }

        // jeśli wiadomość nie nasza — standardowa odpowiedź o nieaktualnym menu
        try {
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    let ticketChannel;
    try {
      // budujemy permission overwrites warunkowo (aby nie przekazywać null jako id)
  ...
  client.on(Events.InteractionCreate, async (interaction) => {
    ...
    // NEW: Deploy latest commit / render button - usuń wszystkie wiadomości z kanału TICKET_CHANNEL_ID i wyślij nową wiadomość (embed/menu)
    if (action === 'deploy_latest_commit') {
      if (!interaction.member.roles.cache.has(ADMIN_ROLE_ID) && !interaction.member.roles.cache.has(MOD_BUTTONS_ROLE_ID) && !interaction.member.permissions.has(PermissionsBitField.Flags.ManageGuild)) {
        return (didDefer ? interaction.editReply({ content: '❌ Nie masz uprawnień do wykonania tej operacji.' }) : interaction.reply({ content: '❌ Nie masz uprawnień do wykonania tej operacji.', ephemeral: true })).catch(() => {});
      }

      const targetCh = await client.channels.fetch(TICKET_CHANNEL_ID).catch(() => null);
      if (!targetCh) {
        return (didDefer ? interaction.editReply({ content: '❌ Nie znaleziono kanału docelowego.' }) : interaction.reply({ content: '❌ Nie znaleziono kanału docelowego.', ephemeral: true })).catch(() => {});
      }

      try {
        await cleanSetupMessages(targetCh);
        const sent = await sendSetupMessage(targetCh);
        await sendLog('Deploy: odświeżono menu', `Menu zostało zdeployowane przez <@${interaction.user.id}>.`, [
          { name: 'Kanał', value: `<#${TICKET_CHANNEL_ID}>` },
          { name: 'Wysłano wiadomość', value: sent ? `ID: ${sent.id}` : 'brak' }
        ], 'Blue');
        return (didDefer ? interaction.editReply({ content: '✅ Kanał został wyczyszczony i nowa wiadomość została wysłana.' }) : interaction.reply({ content: '✅ Kanał został wyczyszczony i nowa wiadomość została wysłana.', ephemeral: true })).catch(() => {});
      } catch (err) {
        console.error('[ticket] deploy_latest_commit error:', err);
        return (didDefer ? interaction.editReply({ content: '❌ Wystąpił błąd podczas deployu.' }) : interaction.reply({ content: '❌ Wystąpił błąd podczas deployu.', ephemeral: true })).catch(() => {});
      }
    }
