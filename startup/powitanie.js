...
module.exports = (client) => {
    client.on(Events.GuildMemberAdd, async (member) => {
        try {
            ...
            const welcomeMessage = `${emojiWelcome} Witaj <@${member.id}> na serwerze Sprawdziany & Kartkówki!`;

            const embed = new EmbedBuilder()
                .setColor('#0004ffff')
                .setDescription(
                    `> ${welcomeMessage}\n\n` +
                    `> ${emojiRules} Zapoznaj się z naszym regulaminem na: <#${RULES_CHANNEL_ID}>\n` +
            ...
            await channel.send({
                content: '', // ping użytkownika już w embedzie
                embeds: [embed],
                components: [row]
            });
